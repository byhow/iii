<!-- generated by iii-skill-render. DO NOT EDIT (changes here are overwritten on the next render). Edit docs/creating-workers/functions.mdx. -->
...
`metadata` is an arbitrary JSON object stored with the function at registration. The engine never
interprets it, it's stored with the function so other parts of the system can act on it. It surfaces
in `engine::functions::info`, the iii console, and is commonly used with `iii-worker-manager` / Role
based access control.

Because the shape is entirely yours, metadata is a facility that can enable and enhance other
functionality. For example,
[worker-manager RBAC allowlists](./worker-manager#trusted-and-rbac-listeners) can expose functions
...
<Tabs>
  <Tab title="Node / TypeScript">
    ```typescript
    worker.registerFunction("math::add", handler, {
      metadata: { owner: "math-team", public: true },
    });
    ```
...
Distinct from the registration metadata above, each invocation can carry its own optional
`metadata` sidecar: arbitrary JSON that travels as a separate channel from the payload, never
folded into it. The engine sets it at fire time from a trigger's registered `metadata` (see
[trigger metadata](../using-iii/triggers#trigger-metadata)), or a caller attaches it directly on
`trigger({ …, metadata })`. Handlers opt in; existing handlers keep working unchanged.

<Tabs>
  <Tab title="Node / TypeScript">
    ```typescript
    // The optional second argument is `undefined` when the invocation carries none.
    worker.registerFunction("audit::write", async (data, metadata) => {
  ...
  <Tab title="Python">
    ```python
    # Forwarded only to handlers that declare a parameter named `metadata`
    # (positional or keyword-only); other signatures are called as before.
    async def write(data, metadata=None):
        return {"ok": True, "metadata": metadata}

    worker.register_function("audit::write", write)
    ```

  </Tab>
  <Tab title="Rust">
    ```rust
    // Two-argument closures receive the sidecar as `Option<Value>`;
    // one-argument closures keep working unchanged.
    worker.register_function(
        "audit::write",
        RegisterFunction::new_async(|input: Value, metadata: Option<Value>| async move {
            Ok(serde_json::json!({ "ok": true, "metadata": metadata }))
        }),
    );
    ```
